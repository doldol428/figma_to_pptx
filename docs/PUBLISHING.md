...
**완료됨.** `manifest.json` 에 발급받은 ID 가 들어가 있다.

```json
"id": "1669210357172947016"
```

이 ID 는 발급한 Figma 계정에 묶여 있고 영구적이다. 게시도 그 계정으로 해야 한다.
아래 절차는 다른 플러그인을 새로 만들 때를 위한 기록이다.

<details>
<summary>ID 를 새로 발급받는 절차</summary>
...
4. 템플릿은 **Default**(A blank slate for your imagination) 선택
   — 세 개 중 무엇을 골라도 상관없다. 생성된 파일은 전부 버리고 `id` 만 꺼내 쓴다
5. **Save as** → 저장 위치를 묻는다. 바탕화면 등 **임시 폴더**를 지정한다.
   이 저장소 안에 만들면 안 된다
...
ID 를 바꾸면 Figma 는 **다른 플러그인**으로 인식한다. 개발 목록에 이전 항목과 껍데기 항목이
남으므로 둘 다 지워야 한다.

1. **Plugins → Development → Manage plugins in development**
2. 이전 항목과 방금 만든 껍데기 항목 제거 (같은 이름이 여러 개 보인다)
3. **Import plugin from manifest…** 로 이 저장소의 `manifest.json` 을 다시 등록
4. 실행해서 정상 동작 확인

ID 는 비밀이 아니다. Community URL 에 그대로 노출되는 공개 값이라 저장소에 커밋해야 한다.
그래야 다른 PC 에서 clone 했을 때 같은 플러그인으로 잡힌다.
