...
그래서 **프레임 비율이 표준 슬라이드 비율과 일치할 때만** 표준 크기 옵션을 함께 제시하고,
그중 하나를 기본값으로 고른다.

| 프레임 | 기본 선택 | 결과 |
|---|---|---|
| 1920 × 1080 px | PowerPoint 와이드스크린 16:9 | 33.87 × 19.05 cm (균등 배율 0.5×) |
| 1280 × 720 px | PowerPoint 와이드스크린 16:9 | 33.87 × 19.05 cm (0.75×) |
| 1024 × 768 px | PowerPoint 표준 4:3 | 25.4 × 19.05 cm (0.703×) |
| 1080 × 1920 px | PowerPoint 와이드스크린 세로 | 19.05 × 33.87 cm (0.5×) |
| 960 × 540 px | 프레임 실측 | 이미 표준 크기라 배율 없음 |
| 595.28 × 841.89 px (A4) | 프레임 실측 | 210 × 297 mm — 맞는 표준이 없음 |
...
드롭다운에서 언제든 "프레임 실측"으로 되돌릴 수 있다. 실제 인쇄물 크기로 만든 16:9 사이니지처럼
물리 크기가 의미 있는 경우가 여기 해당한다.
...
- 220 DPI 요청이 세 프리셋(1× / 0.5× / 0.25×) 모두에서 실효 220 DPI 로 떨어지는지,
...
src/
  shared/
    units.ts      1px = 1pt 기준 환산. 용지 규격 감지(표시용)
    presets.ts    슬라이드 크기 프리셋 — 비율이 맞는 표준만 제시, 균등 배율 계산
